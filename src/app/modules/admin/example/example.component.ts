  ...
  this._inventoryService.getInventories(null)
    ...
    .subscribe({
      ...
      error: (err) => {
        ...
        this._notificationService.error('Erreur lors du chargement des inventaires.');
        this.allInventories = [];
        this.inventories = [];
        this.loading = false;
      }
    ...
    onCreate(): void {
        ...
        dialogRef.afterClosed().subscribe(result => {
            ...
            if (result === 'created') {
                ...
                this._notificationService.success('Entrepôt créé avec succès !');
                this.loadInventories();
            }
        ...
        this._inventoryService.setActive(id, !currentStatus)
            ...
            .subscribe({
                next: () => {
                    ...
                    console.log(`Statut de l'entrepôt ${id} mis à jour.`);
                    this._notificationService.success(`Statut de l'entrepôt mis à jour.`);
                },
                error: (err) => {
                    console.error('Error toggling warehouse status:', err);
                    this._notificationService.error('Erreur lors de la mise à jour du statut.');
                }
            });
    }
}
