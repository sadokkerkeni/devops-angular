    ...
    this._returnLineService.getAll()
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          let errorMsg = 'Erreur lors du chargement des retours.';
          if (err.status === 403) {
            errorMsg = 'Accès refusé: vérifiez vos droits (rôle requis).';
  ...
  <div class="dialog-content flex-auto overflow-y-auto p-6 md:p-8">
    <form [formGroup]="returnLineForm" (ngSubmit)="onSubmit()" class="flex flex-col">
      ...
      <mat-form-field class="w-full">
        ...
        <input matInput [matDatepicker]="picker" formControlName="dateRetour" required>
      ...
      <mat-form-field class="w-full mt-4">
        ...
        <mat-error *ngIf="returnLineForm.get('quantite')?.invalid && returnLineForm.get('quantite')?.touched">
          <span *ngIf="returnLineForm.get('quantite')?.errors?.['required']">La quantité est requise.</span>
          <span *ngIf="returnLineForm.get('quantite')?.errors?.['min']">La quantité doit être supérieure à 0.</span>
        </mat-error>
      </mat-form-field>
      ...
      <mat-form-field class="w-full mt-4">
        ...
        <mat-select formControlName="articleId" placeholder="Sélectionnez un article" required>
        ...
        <mat-error *ngIf="returnLineForm.get('articleId')?.invalid && returnLineForm.get('articleId')?.touched">
          <span *ngIf="returnLineForm.get('articleId')?.errors?.['required']">L'article est requis.</span>
        </mat-error>
      </mat-form-field>
      ...
      <mat-form-field class="w-full mt-4">
        ...
        <mat-select formControlName="userId" placeholder="Sélectionnez un utilisateur" required>
          <mat-option *ngFor="let user of users" [value]="user.id">
            {{ user.firstName }} {{ user.lastName }} ({{ user.email }})
          </mat-option>
        </mat-select>
        <mat-error *ngIf="returnLineForm.get('userId')?.invalid && returnLineForm.get('userId')?.touched">
          <span *ngIf="returnLineForm.get('userId')?.errors?.['required']">L'utilisateur est requis.</span>
        </mat-error>
      </mat-form-field>
      ...
      <mat-form-field class="w-full mt-4">
        ...
        <mat-select formControlName="statusId" placeholder="Sélectionnez un statut" required>
          <mat-option *ngFor="let status of statuses" [value]="status.id">
            {{ status.description }}
          </mat-option>
        </mat-select>
        <mat-error *ngIf="returnLineForm.get('statusId')?.invalid && returnLineForm.get('statusId')?.touched">
          <span *ngIf="returnLineForm.get('statusId')?.errors?.['required']">Le statut est requis.</span>
        </mat-error>
      </mat-form-field>
...
export class ReturnLineCreateComponent implements OnInit {
  ...
  constructor(
    ...
    this.returnLineForm = this._formBuilder.group({
      ...
      statusId: [null, [Validators.required, Validators.min(1)]]  // Status initial, ex: 1 = "En Attente"
    });
  }

  ngOnInit(): void {
    // Load articles, users and statuses
    this.loadArticles();
    this.loadUsers();
    this.loadStatuses();
  }
  ...
  loadArticles(): void {
    this._http.get<any[]>('http://localhost:5288/api/Articles')
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe({
        next: (data) => {
          this.articles = data || [];
        ...
        error: (err) => {
          console.error('[ReturnLineCreateComponent] Erreur lors du chargement des articles:', err);
  ...
  loadUsers(): void {
    this._http.get<any[]>('http://localhost:5288/api/users/admin-all')
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe({
        next: (data) => {
          this.users = data || [];
        ...
        error: (err) => {
          console.error('[ReturnLineCreateComponent] Erreur lors du chargement des utilisateurs:', err);
          this._snackBar.open('Erreur lors du chargement des utilisateurs.', 'Erreur', { duration: 5000 });
        }
      });
  }

  loadStatuses(): void {
    this._http.get<any[]>('http://localhost:5288/api/status')
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe({
        next: (data) => {
          this.statuses = data || [];
        ...
        error: (err) => {
          console.error('[ReturnLineCreateComponent] Erreur lors du chargement des statuts:', err);
          this._snackBar.open('Erreur lors du chargement des statuts.', 'Erreur', { duration: 5000 });
        }
      });
  }
...
export class ReturnLineEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Load articles, users and statuses first
    this.loadArticles();
    this.loadUsers();
    this.loadStatuses();
  ...
  loadArticles(): void {
    this._http.get<any[]>('http://localhost:5288/api/Articles')
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('[ReturnLineEditComponent] Erreur lors du chargement des articles:', err);
          this._snackBar.open('Erreur lors du chargement des articles.', 'Erreur', { duration: 5000 });
          this.loadingData = false;
        }
      });
  ...
  loadUsers(): void {
    this._http.get<any[]>('http://localhost:5288/api/users/admin-all')
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('[ReturnLineEditComponent] Erreur lors du chargement des utilisateurs:', err);
          this._snackBar.open('Erreur lors du chargement des utilisateurs.', 'Erreur', { duration: 5000 });
        }
      });
  }
  ...
  loadStatuses(): void {
    this._http.get<any[]>('http://localhost:5288/api/status')
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('[ReturnLineEditComponent] Erreur lors du chargement des statuts:', err);
          this._snackBar.open('Erreur lors du chargement des statuts.', 'Erreur', { duration: 5000 });
        }
      });
  }
