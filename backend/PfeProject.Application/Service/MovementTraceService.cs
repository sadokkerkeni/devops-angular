    ...
    public class MovementTraceService : IMovementTraceService
    {
        ...
        public async Task<ReturnLineReadDto?> CreateReturnLineAndAddStockAsync(int movementTraceId, int userId)
        {
            ...
            var returnLineCreateDto = new ReturnLineCreateDto
            {
                ...
                Quantite = movementTrace.Quantite, // Now int, no need for ??
                ArticleId = articleId,
            ...
            // Quantite is now int, no need to parse
            int quantityToAdd = movementTrace.Quantite;
            ...
            var stockUpdated = await _sapService.AddStockAsync(movementTrace.UsNom ?? string.Empty, quantityToAdd);
            if (!stockUpdated)
            {
        ...
        {
            Console.WriteLine($"[MovementTraceService] Création d'un MovementTrace pour CompanyId {companyId}: UsNom={dto.UsNom}, Quantite={dto.Quantite}, UserId={dto.UserId}, DetailPicklistId={dto.DetailPicklistId}");
            ...
            await _repository.AddAsync(mt);
            Console.WriteLine($"[MovementTraceService] ✅ MovementTrace créé avec succès, ID: {mt.Id}, DateMouvement: {mt.DateMouvement}");
        ...
        public async Task<ReturnLineReadDto?> CreateReturnLineAndAddStockForCompanyAsync(int movementTraceId, int userId, int companyId)
        {
            ...
            // 4. Créer le ReturnLine via le ReturnLineService - Company-aware
            Console.WriteLine($"[MovementTraceService] Création du ReturnLine pour MovementTrace ID {movementTraceId} dans l'entreprise {companyId}.");
            var createdReturnLine = await _returnLineService.CreateForCompanyAsync(returnLineCreateDto, companyId);
            if (createdReturnLine == null)
            {
                Console.WriteLine($"[MovementTraceService] ❌ Échec de la création du ReturnLine pour MovementTrace ID {movementTraceId} dans l'entreprise {companyId}.");
                return null;
            }
            Console.WriteLine($"[MovementTraceService] ✅ ReturnLine créé avec succès, ID: {createdReturnLine.Id}");

            // 5. Mettre à jour le stock SAP - Company-aware
            // Quantite is now int, no need to parse
            int quantityToAdd = movementTrace.Quantite;

            Console.WriteLine($"[MovementTraceService] Tentative de mise à jour du stock SAP pour US '{movementTrace.UsNom}' avec quantité {quantityToAdd} dans l'entreprise {companyId}.");
            var stockUpdated = await _sapService.AddStockForCompanyAsync(movementTrace.UsNom ?? string.Empty, quantityToAdd, companyId);
            if (!stockUpdated)
            {
                Console.WriteLine($"[MovementTraceService] ❌ Échec de la mise à jour du stock SAP pour US '{movementTrace.UsNom}' dans l'entreprise {companyId}.");
                // Note: On continue quand même car le ReturnLine a été créé
            }
            else
            {
                Console.WriteLine($"[MovementTraceService] ✅ Stock SAP mis à jour avec succès pour US '{movementTrace.UsNom}' dans l'entreprise {companyId}.");
            }
