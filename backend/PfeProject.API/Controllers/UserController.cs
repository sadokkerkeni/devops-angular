...
namespace PfeProject.API.Controllers
{
    ...
    public class UsersManagementController : ControllerBase
    {
        ...
        public UsersManagementController(IUserRepository userRepository, IUserRoleRepository userRoleRepository, IRoleRepository roleRepository)
        {
        ...
        public async Task<IActionResult> AdminCreateUser([FromBody] UserCreateDto dto)
        {
            ...
            // Get admin ID for role assignment
            ...
            // ✅ NEW: Assign role if provided
            if (!string.IsNullOrEmpty(dto.Role))
            {
                var role = await _roleRepository.GetByNameAndCompanyAsync(dto.Role, companyId);
                if (role != null)
                {
                    var userRole = new UserRole
                    {
                        UserId = user.Id,
                        RoleId = role.Id,
                        AssignedById = adminId > 0 ? adminId : user.Id,
                        AssignmentDate = DateTime.UtcNow,
                        IsActive = true,
                        Note = "Rôle assigné lors de la création de l'utilisateur",
                        CompanyId = companyId
                    };
                    await _userRoleRepository.AddAsync(userRole);
                    Console.WriteLine($"[UsersManagementController] Role '{dto.Role}' assigned to new user {user.Id}");
                }
                else
                {
                    Console.WriteLine($"[UsersManagementController] Role '{dto.Role}' not found for company {companyId}");
                }
            }
